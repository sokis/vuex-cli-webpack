...
var config = {
  ...
  compiler_vendor: ['history', 'vue', 'vue-router', 'vuex', 'vuex-localstorage', 'vuex-promise'],
...
try {
  ...
  if (_fs2.default.existsSync((0, _path.join)(config.path_base, 'webpack.config.js'))) {
    debug('Merge webpack config .');
    var _environments = require(process.env.PWD + '/webpack.config');
    var _overrides = _environments[config.env];

    if (_overrides) {
      (0, _assign2.default)(config, _overrides(config));
    }
  }
} catch (error) {}
